...
write.csv(dd,"MDS/mean_sd_rf.csv",row.names = FALSE, quote = FALSE)
...
write.csv(total_res, "MDS/dispersion_rf_path.csv", row.names = FALSE, quote = FALSE)
...
p2 <- ggplot(df_rf, aes(x = X, y = Y, color = Type, shape = Type)) +
  ...
  theme_bw() +
  labs(
    title = "Gt0.8 (RF Distance)",
    x = "Dimension 1",
    y = "Dimension 2"
  )+
  theme(legend.position = c(0.88,0.85),
        legend.background = element_blank())+
  annotate("text", x = 0.9*min(df_rf$X), y = 0.9*max(df_rf$Y), 
...
p3 <- ggplot(df_path, aes(x = X, y = Y, color = Type, shape = Type)) +
  ...
  theme_bw() +
  labs(
    title = "Gt0.8 (Path Distance)",
    x = "Dimension 1",
    y = "Dimension 2"
  )+
  theme(legend.position = c(0.88,0.85),
        legend.background = element_blank())+
  annotate("text", x = 0.9*min(df_path$X), y = 0.9*max(df_path$Y), 
           label = text_label, hjust = 0, vjust = 1, size = 4)
  # xlim(-20, 25) + 
  # ylim(-20, 20)
...
ggsave("MDS/RF_MDS.pdf", p2, width = 10, height = 8)
ggsave("MDS/PATH_MDS.pdf", p3, width = 10, height = 8)
