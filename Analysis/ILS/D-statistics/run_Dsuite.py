...
def run_snp_sites(fasta_file, snp_sites_path_or_image):
    ...
    # Post-process VCF to thin SNPs to 1 SNP per 500 base pairs, or approx. 2 SNPs per locus on average
    ...
    with open(output_vcf, 'w') as vcf_out:
        ...
        for line in lines:
            if line.startswith("#"):  # Write header lines directly
                vcf_out.write(line)
            else:
                parts = line.split('\t')
...
if __name__ == "__main__":
    ...
    for i, combo in enumerate(combinations_of_three):
        ...
        run_snp_sites(output_fasta, snp_sites_path_or_image)
        
        # Run Dsuite on the VCF file generated by snp-sites
        vcf_file = f"{output_fasta}_snps.vcf"
